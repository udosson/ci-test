...
docker exec -t cli /bin/bash ./network.sh up

echo "Starting orderers, peers, and couchdb..."
sh ./scripts/startAndConnectNetwork.sh

echo "Creating the channel..."
docker exec -t cli /bin/bash ./network.sh createChannel

echo "Install ext chaincode..."
docker exec -t cli /bin/bash ./scripts/installChaincode.sh 1 2
docker exec -t cli /bin/bash ./scripts/installChaincode.sh 1 1

echo "Starting ext chaincode..."
sh ./scripts/startExtChaincode.sh

echo "Deploying CC Ext..."
docker exec -t cli /bin/bash ./scripts/deployCCExt.sh 1 1
docker exec -t cli /bin/bash ./scripts/deployCCExt.sh 1 2

sleep 20
docker exec -t cli /bin/bash ./scripts/commitCCExt.sh 1 1
